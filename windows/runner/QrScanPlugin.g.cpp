...
int64_t SaveQrScan::date() const {
  return date_;
}

void SaveQrScan::set_date(int64_t value_arg) {
  date_ = value_arg;
}
...
void QrApi::SetUp(
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.qr_scan_plugin_package.QrApi.getListScan" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          api->GetListScan([reply](ErrorOr<EncodableList>&& output) {
            if (output.has_error()) {
